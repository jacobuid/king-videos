...
async function pinHash(pin:string,salt:Uint8Array){const material=await crypto.subtle.importKey('raw',encoder.encode(pin),'PBKDF2',false,['deriveBits']);return new Uint8Array(await crypto.subtle.deriveBits({name:'PBKDF2',hash:'SHA-256',salt:new Uint8Array(salt).buffer,iterations:120000},material,256))}
...
export default{async fetch(request:Request,env:Env):Promise<Response>{
  ...
  try{
    ...
    if(path==='/api/profiles'&&request.method==='POST'){
      ...
      if(pin){const salt=crypto.getRandomValues(new Uint8Array(16));saltValue=base64url(salt);hash=base64url(await pinHash(pin,salt))}
    ...
    if(profileMatch&&request.method==='POST'){
      ...
      let hash:string|null=null,saltValue:string|null=null;if(pin){const salt=crypto.getRandomValues(new Uint8Array(16));saltValue=base64url(salt);hash=base64url(await pinHash(pin,salt))}
