...
const validAvatar=(avatar:string)=>/^(amy|bingo|blaze|blue|bluey|bucket-n-shovel|bunnie|daphane|fred|julie-su|knuckles|magenta|mailbox|sally|salt-n-pepper|scoobydoo|shadow|shaggy|soapy|sonic|tails|velma)\.png$/.test(avatar)
...
export default{async fetch(request:Request,env:Env):Promise<Response>{
  ...
  try{
    ...
    const profileMatch=path.match(/^\/api\/profiles\/([^/]+)\/(unlock|pin|type|avatar)$/)
    if(profileMatch&&request.method==='POST'){
      ...
      const body=await request.json()as{pin?:string;avatar?:string;isKids?:boolean}
      ...
      if(profileMatch[2]==='avatar'){const avatar=body.avatar?.trim()||'';if(!validAvatar(avatar))return withCors(json({error:'Choose a valid profile picture.'},400),cors);await env.DB.prepare('UPDATE profiles SET avatar=? WHERE id=? AND user_id=?').bind(avatar,profile.id,userId).run();return withCors(json({avatar}),cors)}
    ...
    if(path==='/api/progress'&&request.method==='GET'){const profileId=url.searchParams.get('profileId');if(!await authorizedProfile(request,env,userId,profileId))return withCors(json({error:'Profile is locked.'},403),cors);return withCors(json((await env.DB.prepare('SELECT media_id AS mediaId,position_seconds AS positionSeconds,updated_at AS updatedAt FROM progress WHERE profile_id=? ORDER BY updated_at DESC').bind(profileId).all()).results),cors)}
    if(path==='/api/favorites'&&request.method==='GET'){const profileId=url.searchParams.get('profileId');if(!await authorizedProfile(request,env,userId,profileId))return withCors(json({error:'Profile is locked.'},403),cors);return withCors(json((await env.DB.prepare('SELECT media_id AS mediaId FROM favorites WHERE profile_id=? ORDER BY sort_order,rowid').bind(profileId).all()).results),cors)}
    if(path==='/api/favorites/reorder'&&request.method==='POST'){const body=await request.json()as{profileId?:string;mediaIds?:string[]};if(!await authorizedProfile(request,env,userId,body.profileId))return withCors(json({error:'Profile is locked.'},403),cors);if(!Array.isArray(body.mediaIds)||body.mediaIds.length>500||new Set(body.mediaIds).size!==body.mediaIds.length||body.mediaIds.some(id=>typeof id!=='string'||!id||id.length>200))return withCors(json({error:'Invalid My List order.'},400),cors);await env.DB.batch(body.mediaIds.map((mediaId,index)=>env.DB.prepare('UPDATE favorites SET sort_order=? WHERE profile_id=? AND media_id=?').bind(index,body.profileId,mediaId)));return withCors(json({ok:true}),cors)}
    if(path==='/api/favorites'&&(request.method==='POST'||request.method==='DELETE')){const body=await request.json()as{profileId?:string;mediaId?:string};if(!await authorizedProfile(request,env,userId,body.profileId))return withCors(json({error:'Profile is locked.'},403),cors);if(!body.mediaId||body.mediaId.length>200)return withCors(json({error:'Invalid video.'},400),cors);if(request.method==='POST')await env.DB.prepare('INSERT OR IGNORE INTO favorites(profile_id,media_id,sort_order) VALUES(?,?,COALESCE((SELECT MAX(sort_order)+1 FROM favorites WHERE profile_id=?),0))').bind(body.profileId,body.mediaId,body.profileId).run();else await env.DB.prepare('DELETE FROM favorites WHERE profile_id=? AND media_id=?').bind(body.profileId,body.mediaId).run();return withCors(json({ok:true}),cors)}
