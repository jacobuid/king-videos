...
for(const file of files){
  ...
  const sql=`INSERT INTO media(id,title,description,category,video_key,source_url,thumbnail_key,mime_type,kids_allowed,series_id,season_number,episode_number,genres,rating,featured) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET title=excluded.title,description=excluded.description,category=excluded.category,source_url=excluded.source_url,thumbnail_key=excluded.thumbnail_key,mime_type=excluded.mime_type,kids_allowed=excluded.kids_allowed,series_id=excluded.series_id,season_number=excluded.season_number,episode_number=excluded.episode_number,genres=excluded.genres,rating=excluded.rating,featured=excluded.featured`
  await query(sql,[id,title,manifest.description||'',manifest.category||'tv',`external/archive/${identifier}/${file.name}`,sourceUrl,thumbnailKey,'video/mp4',manifest.kids?1:0,manifest.id,season,episode,JSON.stringify(manifest.genres||[]),manifest.rating||null,manifest.featured?1:0])
