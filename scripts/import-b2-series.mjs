...
async function uploadBytes(bytes,key,type){const response=await fetch(upload.uploadUrl,{method:'POST',headers:{Authorization:upload.authorizationToken,'X-Bz-File-Name':encodeURIComponent(key),'Content-Type':type,'Content-Length':String(bytes.length),'X-Bz-Content-Sha1':createHash('sha1').update(bytes).digest('hex')},body:bytes});if(response.status===401){upload=await b2('b2_get_upload_url',{bucketId:bucket.bucketId});return uploadBytes(bytes,key,type)}if(!response.ok)throw new Error(`Upload failed for ${key}: ${await response.text()}`)}
