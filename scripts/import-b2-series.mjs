...
for(const name of names){
  ...
  if(!match&&!numbered&&!special&&!short){console.log(`Skipping unrecognized video: ${name}`);continue}
  const sourceTitle=(match?.[3]||numbered?.[2]||special?.[1]||short?.[1]).trim(),metadata=episodeMetadata.get(normalizeTitle(sourceTitle)),season=metadata?.season??(match?Number(match[1]):special||short?0:1),episode=metadata?.episode??(match?Number(match[2]):numbered?Number(numbered[1]):imported+1),title=metadata?.title||sourceTitle,slug=title.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,''),id=`${manifest.id}-s${String(season).padStart(2,'0')}e${String(episode).padStart(3,'0')}-${slug}`,videoKey=`movies/${manifest.id}/season-${season}/${id}.mp4`
  ...
  await query(sql,[id,title,metadata?.description||manifest.description||'',manifest.category||'tv',videoKey,thumbnailKey,subtitleKey,'video/mp4',manifest.kids?1:0,manifest.id,manifest.title,season,episode,metadata?.year||manifest.year||null,JSON.stringify(manifest.genres||[]),manifest.rating||null,manifest.featured?1:0]);imported++;console.log(`Imported Season ${season}, Episode ${episode}: ${title}`)
}
